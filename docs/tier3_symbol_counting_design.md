...
- **M3 — VLM verifier (opt-in).** ✅ `count_verifier`: `CountVerifier` (injectable client + grounding + graceful degradation, like `section_llm`) + `merge`/`reconcile`/`flip_fixture_items` + `StubVerifierClient`/`ReplayClient`/`RecordingClient` for the offline suite + real `AnthropicVerifierClient` (vision, `claude-opus-4-8`, lazy `anthropic` import, opt-in). Merge never overwrites the deterministic count: agreement raises confidence and flips `unknown_plan_count → plan_count`; disagreement keeps the count, drops confidence, records the model's read; error degrades to unverified. `reconcile` takes the MAX sheet count as the conservative building total and flags multi-sheet disagreement (deferred dedup, per the M0 decision). The committed `schedule_items.json` is NOT re-generated with the verifier: the stub would fake "verified" and the real VLM needs credentials/network — so the artifact stays at M1's deterministic candidate counts; verification runs opt-in.
