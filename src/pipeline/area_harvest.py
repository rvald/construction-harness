...
def harvest_room_areas(pdf_path, known_rooms: set[str], page_range=None,
                       file_id: str | None = None, **join_kw) -> list[RoomArea]:
    """Driver: locate area plans in a page range and harvest per-room areas from them.

    fitz-only and fast. Areas are de-duplicated by room across pages (first area plan
    wins); each result records its source page. Rooms with no nearby label are simply
    ...
    with fitz.open(str(pdf_path)) as doc:
        ...
        pages_tokens = [(i, positioned_tokens(doc[i])) for i in rng]
    ...
    for i in locate_area_plans(pages_tokens, known_rooms):
        ...
        for ra in join_areas(room_tokens(toks, known_rooms), sf_labels(toks), **join_kw):
            ...
            ra.source = {"file_id": fid, "page_index": i}
            out.append(ra)
    return out
