...
def extract_schedule_items(source, page_range=None,
                           schemas: list[ScheduleSchema] | None = None) -> list[ScheduleItem]:
    """Find and parse schedules across a page range, keyed by title signature.

    `source` is a path OR an already-open Document. Page discovery is by signature +
    header coverage (no page constants): a page is a candidate for a schema only if its
    text carries the schema's title tokens, and only tables that actually resolve to the
    schema are parsed. The cheap text gate uses fitz (memoized); the expensive pdfplumber
    extract_tables runs only on candidate pages (Page.tables opens pdfplumber lazily, so
    with no candidates it never opens). Items are de-duplicated by (schedule, mark) so a
    schedule referenced on several pages isn't double-counted.
    """
    schemas = schemas or SCHEDULE_REGISTRY
    with using_document(source) as doc:
        # 1) Fast text pass: which pages are candidates for which schemas.
        candidates: dict[int, list[ScheduleSchema]] = {}
        rng = page_range if page_range is not None else range(doc.page_count)
        for i in rng:
            matched = [s for s in schemas if _page_matches(doc.page(i).get_text(), s)]
            if matched:
                candidates[i] = matched

        # 2) Tables only on candidate pages — the SAME cached Page, so pdfplumber opens
        #    lazily on the first .tables() and only if a candidate exists.
        seen: set[tuple[str, str]] = set()
        items: list[ScheduleItem] = []
        for i in sorted(candidates):
            tables = doc.page(i).tables()
            for schema in candidates[i]:
                src = {"file_id": doc.file_id, "page_index": i, "schedule": schema.name}
